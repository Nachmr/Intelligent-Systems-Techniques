...
(:task mover-avion
 ...
 (:method hay-fuel-zoom ;; este método se escogerá para usar la acción zoom siempre que el avión tenga fuel para
                          ;; volar desde ?c1 a ?c2
  ...
  :precondition (and (hay-fuel ?a ?c1 ?c2) (<=(total-fuel-used)(fuel-limit)))
  :tasks (
          (zoom ?a ?c1 ?c2)
         )
   )
  (:method no-hay-fuel-zoom ;; este método se escogerá cuando no haya fuel suficiente.
                           ;;primero repostará y después realizará el vuelo.
  :precondition (and (not(hay-fuel ?a ?c1 ?c2)) (<=(total-fuel-used)(fuel-limit)))
  :tasks (
          (refuel ?a ?c1)
          (zoom ?a ?c1 ?c2)
         )
   )
   (:method hay-fuel-fly ;; este método se escogerá para usar la acción fly siempre que el avión tenga fuel para
                          ;; volar desde ?c1 a ?c2
  :precondition (and (hay-fuel ?a ?c1 ?c2) (<=(total-fuel-used)(fuel-limit)))
  :tasks (
          (fly ?a ?c1 ?c2)
  ...
  (:method no-hay-fuel-fly ;; este método se escogerá cuando no haya fuel suficiente.
                           ;;primero repostará y después realizará el vuelo.
  :precondition (and (not(hay-fuel ?a ?c1 ?c2)) (<=(total-fuel-used)(fuel-limit)))
  :tasks (
          (refuel ?a ?c1)
